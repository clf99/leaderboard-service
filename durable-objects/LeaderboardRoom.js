const ATTRITION_MS = 7 * 24 * 60 * 60 * 1000; // 7 days in ms
...
export class LeaderboardRoom {
  ...
  async fetch(request) {
    ...
    if (method === 'GET') {
      ...
      const enriched = scores.map(entry => ({
        playerId: entry.playerId,
        displayName: entry.displayName,
        score: entry.score,
        recordedAt: entry.recordedAt,
        source: entry.source || 'anonymous',
        nostrNpub: entry.nostrNpub || null,
        ageMs: now - new Date(entry.recordedAt).getTime(),
        dead: entry.source === 'nostr'
          ? false
          : (now - new Date(entry.recordedAt).getTime()) >= ATTRITION_MS
      }));

      return new Response(JSON.stringify({ gameId, scores: enriched }), { status: 200, headers });
    }
    ...
    if (method === 'POST') {
      ...
      const nostrNpub = String(body?.nostrNpub || '').trim().slice(0, 80) || null;
      ...
      const source = nostrNpub && nostrNpub.startsWith('npub1') ? 'nostr' : 'anonymous';
      ...
      const now = Date.now();
      const result = trimmed.map(entry => ({
        ...entry,
        ageMs: now - new Date(entry.recordedAt).getTime(),
        dead: entry.source === 'nostr'
          ? false
